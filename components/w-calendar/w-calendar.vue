<template>
	<view class="w-calendar">
		<view class="w-calendar-header">
			<u-icon name="arrow-left" size="20" color="#333" @tap="prev"></u-icon>
			<!-- <view class="prev" @tap="prev"></view> -->
			<view class="current">{{year}}年{{month}}月</view>
			<!-- <view class="next" @tap="next"></view> -->
			<u-icon name="arrow-right" size="20" color="#333" @tap="next"></u-icon>
				...
				<view class="w-calendar-item">
					...
					<view class="w-calendar-day">
						<view class="w-day-item" :class="[
								...
								{'active':item.active},
								{'select':item.select},
								{'disabled-select':item.selectDisabled},
								{'disabled-before':disabledBefore&&item.disabledBefore},
								{'disabled-after':disabledAfter&&item.disabledAfter}]" v-for="(item,dIndex) in month.data" :key="dIndex"
							@tap="checkItem(item)">
							<view class="box">
								<text>{{item.day}}</text>
	...
	export default {
		...
		methods: {
			...
			prev() {
				...
				this.init("prev")
				// console.log(this.dataList[this.swiperIndex])
				// this.$emit("confirm", this.dataList[this.swiperIndex]);
			},
			next() {
			...
			init(str) {
				...
				this.dataList.map((v, k) => {
					v.data.map((i, j) => {
						let beforeFlag = new Date(i.year, i.month - 1, i.day).getTime() - new Date(dDate
								.getFullYear(), dDate.getMonth(), dDate.getDate()).getTime() >= 0 ? false :
							true;
						let afterFlag = new Date(i.year, i.month - 1, i.day).getTime() - new Date(dDate
								.getFullYear(), dDate.getMonth(), dDate.getDate()).getTime() > 0 ? true :
							false;
						i.disabledBefore = beforeFlag;
						i.disabledAfter = afterFlag;
						if (i.year == dDate.getFullYear() && i.month == dDate.getMonth() + 1 && i.day ==
							dDate.getDate()) {
							i.active = true;
						}
					})
			...
			initActive(list) {
				...
				const monthData = dataList[this.swiperIndex].data
				// console.log(list)
				monthData.map((i, j) => {
					const data = list.find(item => {
						const time = `${i.year}-${i.month}-${i.day}`
						const tempTime1 = new Date(time).getTime()
						const tempTime = new Date(item.time).getTime()
						return tempTime1 === tempTime
					})
					// console.log(index)
					if (data) {
						if (data.rechargeState == 0) {
							...
							i.selectDisabled = true;
						}
					} else {
						i.select = false;
						i.selectDisabled = false;
					}
				})
				dataList[this.swiperIndex].data = monthData
...
<style lang="scss">
	.w-calendar {
		...
		.w-calendar-header {
			...
			height: 100rpx;
			position: relative;
			// background: linear-gradient(213deg, #0EB83A 0%, #C6FFD5 100%);
			background: #fff;
